...
function verifyTaskSets(gulp, taskSets, skipArrays) {
	...
	taskSets.forEach(function(t) {
		...
		if(isTask && !gulp.hasTask(t)) {
			throw new Error("Task " + t + " is not configured as a task on gulp. If this is a submodule, you may need to use gulp.runSeq.use(gulp). GUH SUH");
		}
...
function runSequence(gulp) {
	...
	function finish(e) {
		...
		var error;
		if(e && e.err) {
			error = new PluginError('gulp.runSeq(' + e.task + ')', e.err, { showStack: options().showErrorStackTrace });
		}
