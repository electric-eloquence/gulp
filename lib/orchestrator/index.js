...
var Orchestrator = function() {
  ...
  delete this.doneCallback; // call this when all tasks in the queue are done
};
...
Orchestrator.prototype.add = function(name) {
  var dep;
  var fn;
  ...
  dep = dep || arguments[1] || [];
  fn = fn || arguments[2] || function() {}; // no-op
  /* istanbul ignore if */
  if (!name) {
...
Orchestrator.prototype.start = function() {
  ...
  if (names.length < 1) {
    // run all tasks
    for (i in this.tasks) {
      if (this.tasks.hasOwnProperty(i)) { // eslint-disable-line no-prototype-builtins
...
Orchestrator.prototype.stop = function(err_, successfulFinish) {
  var err;
  this.isRunning = false;
  if (err_) {
    err = err_;
...
Orchestrator.prototype.allDone = function() {
  var i;
  var task;
  var allDone = true; // nothing disputed it yet
...
Orchestrator.prototype._resetAllTasks = function() {
  ...
  for (task in this.tasks) {
    if (this.tasks.hasOwnProperty(task)) { // eslint-disable-line no-prototype-builtins
...
var eventsArr = ['start', 'stop', 'err', 'task_start', 'task_stop', 'task_err', 'task_not_found', 'task_recursion'];
...
Orchestrator.prototype.onAll = function(callback) {
  ...
  for (i = 0; i < eventsArr.length; i++) {
    listenToEvent(this, eventsArr[i], callback);
  }
};
